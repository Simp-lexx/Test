...
const filterByTeacherIds = (lessons, teacherIds) => lessons.teachers.id.filter((teacher) => {
  return teacherIds.split(',').indexOf(teacher.id) > -1;
});

export const filterLessons = async (lesson, query = {}) => {
  const { where, teacherIds } = getParams(query);
  ...
  // console.log(lessons);
  return teacherIds.length === 0 ? lessons : filterByTeacherIds(lessons, teacherIds);
};
